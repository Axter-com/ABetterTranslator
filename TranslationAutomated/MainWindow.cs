...
namespace TranslationAutomated
{
    ...
    using TranslationAutomated.Properties;
    ...
    using static TranslationAutomated.MainWindow;
	...
	{
        #region DllImport's
        // ToDo: Remove the following DllImport's if they're not used.
        [System.Runtime.InteropServices.DllImport("user32.dll")]
		public static extern IntPtr WindowFromPoint(Point lpPoint);
        [DllImport("user32.dll")]
        [return: MarshalAs(UnmanagedType.Bool)]
        static extern bool IsWindowVisible(IntPtr hWnd);
        //private bool IsThisWindowVisible()
        //{
        ...
        public class ResxMultiThreadTranslator : TranslateMultiThread.MultiThreadTranslator
        {
            ...
            {
                if (TotalCount < 1)
                    Debug.WriteLine($"ShowProgress received unexpected value of {TotalCount}");
            ...
            protected override void TaskComplete(int QtyItemsCompleted, TimeSpan TotalRunTime)
            {
                ...
                if ( _qtySkippedDisplayErrs > 0 )
                    Debug.WriteLine($"Warn: Skipped display {_qtySkippedDisplayErrs} duplicate errors.");
        ...
        public MainWindow()
		{
			...
			string inputPath = Settings.Default.inputBox;
			...
			if ( string.IsNullOrEmpty(Settings.Default.outputBox) )
			{
			...
			if ( Settings.Default.comboBoxFromLanguage > -1 )
				comboBoxFromLanguage.SelectedIndex = comboBoxFromLang.SelectedIndex = Settings.Default.comboBoxFromLanguage;
			else
				_ = GetValid_codeBox_SelectIndex();
            comboBoxToLang.SelectedIndex = comboBoxFromLanguage.SelectedIndex;
        ...
        { // Only use this in debug mode for validating the data in LanguageCode.cs and in LanguageDictionary.cs
            ...
            foreach ( var lang in LanguagesThatNeedToBeChecked)
            {
                if ( !lang.Value)
                {
                    string Issue = $"Error: Did not find tag ({lang.Key}) in any of the LanguageCodes translators.";
                    Debug.WriteLine(Issue + "...");
                    DetectedIssues.Add(Issue);
                }
            }
		...
		private string GetDefaultPath(string DefaultLoadedValue, string SubPathInRoamingApp, string? ParentPath = null)
		{
			...
			// Environment.ExpandEnvironmentVariables("%AppData%\\Roaming\\TranslationAutomated");
			...
			{
				Debug.WriteLine("Error:[GetDefaultPath] Could NOT create path [" + PathToReturn + "]. Exception thrown:" + e.Message);
				return "";
			}
			return PathToReturn;
		}
        private bool CheckIfSrcResxHasAppendedLanguageToBaseFileName()
		{
			if ( !string.IsNullOrEmpty(inputBox.Text) )
			{
				...
				if ( match.Success )
				{
					...
					comboBoxFromLanguage.SelectedIndex = comboBoxFromLang.SelectedIndex = GetLanguageIndex(code);
        ...
        //private void WriteLine(string message, VerbosityLevels verbositylevels)
        //{
        //    switch ( verbositylevels )
        //    {
        //        case VerbosityLevels.Errors:
        //            WriteLine(message, LoggingColorSet.ErrorLogging, verbositylevels);
        //            break;
        //        case VerbosityLevels.Normal:
        //            if ( comboBoxScreenVerbosityLevel.SelectedIndex > 1 )
        //                WriteLine(message, LoggingColorSet.NormalLogging, verbositylevels);
        //            break;
        //        case VerbosityLevels.Detail:
        ...
        private void MainWindow_Load(object sender, EventArgs e)
		{
            ...
            // checkbox settings
            checkBoxAddSubLangToDefaultSet.Checked = Settings.Default.checkBoxAddSubLangToDefaultSet;
            ...
            checkBoxDispalySummary.Checked = Settings.Default.checkBoxDispalySummary;
            checkBoxDispalyWarningPrompts.Checked = Settings.Default.checkBoxDispalyWarningPrompts;
            checkBoxOnStartupSetPreviousSessionLangSet.Checked = Settings.Default.checkBoxOnStartupSetPreviousSessionLangSet;
            checkBoxRecursive.Checked = Settings.Default.checkBoxRecursive;
			...
			textBoxLogFolderPath.Text = GetDefaultPath(Settings.Default.textBoxLogFolderPath, "Log", Path.GetTempPath() + "TranslationAutomated");
            ...
            if ( checkBoxOnStartupSetPreviousSessionLangSet.Checked && SelectedLanguageSet > -1 )
        ...
        private void MainWindow_FormClosing(object sender, FormClosingEventArgs e)
		{
            ...
            // checkbox settings
            Settings.Default.checkBoxAddSubLangToDefaultSet = checkBoxAddSubLangToDefaultSet.Checked;
            ...
            Settings.Default.checkBoxDispalySummary = checkBoxDispalySummary.Checked;
            Settings.Default.checkBoxDispalyWarningPrompts = checkBoxDispalyWarningPrompts.Checked;
            Settings.Default.checkBoxOnStartupSetPreviousSessionLangSet = checkBoxOnStartupSetPreviousSessionLangSet.Checked;
            Settings.Default.checkBoxRecursive = checkBoxRecursive.Checked;
        ...
        private void RunBackgroundTask(MultiThreadTranslator_Options options)
		{
			...
			if ( !success )
			{
				Debug.WriteLine("Error:[RunBackgroundTask] TranslateData failed.");
        ...
        private int GetLanguageIndex(string Tag = "")
        {
            ...
            if ( comboBoxFromLanguage.Items.Contains(GetLanguageNameOfTag(Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName)) )
                return comboBoxFromLanguage.Items.IndexOf(GetLanguageNameOfTag(Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName));
        ...
        private async void TranslateFile()
		{
			...
			while ( dict.MoveNext() )
			{
				if ( dict.Value is string @string && !string.IsNullOrEmpty(@string))
				{
                    ...
                    if (Key.EndsWith(".Text") || Key.EndsWith(".Items") || Key.EndsWith(".ToolTip") || Key.EndsWith(".Items"))
                    {
            ...
            try
            {
                if ( checkBoxBackupFilesBeforeTranslation.Checked )
                {
                    if ( Directory.Exists(textBoxBkUpDir.Text) )
                    {
                        ...
                        foreach ( FileInfo file in di.GetFiles("*.resx", checkBoxRecursive.Checked ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly) )
                        {
                    ...
                    if ( value is string )
                    {
                        ...
                        if ( Key.EndsWith(".Text") || Key.EndsWith(".Items") || Key.EndsWith(".ToolTip") || Key.EndsWith(".Items") )
                        {
                            if ( value.Equals(item.sourceText[idx]) )
                            {
                                ...
                                Debug.WriteLine("Error:[SaveTranslations] Source value did not match Resx file value for item [" + Key + "]. SrcVal=[" + value + "]; ResxVal=[" + item.sourceText[idx] + "]");
                        }
                    }
                    rw.AddResource(node);
                }
        ...
        public void Translator_TaskComplete(int QtyItemsCompleted, TimeSpan totalTranslationTime)
        {
            ...
            LockControls(false, checkBoxDispalySummary.Checked);
            sw.Stop();
            if ( !IsCancel )
            {
            ...
            if ( checkBoxDispalySummary.Checked )
                tabControlTranslateResxSubTab.SelectedIndex = 1;
            else
                tabControlTranslateResxSubTab.SelectedIndex = 0;
		...
		private void PopulateLanguageSet(string[,] languageSet, bool use2LettersOnly = false)
		{
			...
			comboBoxToLang.SelectedIndex = GetLanguageIndex(Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName);
            languageList.Sort(1);
        }
       ...
       private void toolStripMenuItemISO639_1UsingOfficialNames_ShowOnly_Click(object sender, EventArgs e) => toolStripMenuItemISO639_1UsingOfficialNames_Click(sender, e);
        ...
        private static string GetAppendedFileName(string code)
        {
            try
            {
        ...
        private static string GetCulture_2LetterISOName(string code)
        {
            try
            {
                CultureInfo? cultureinfo = CultureInfo.GetCultureInfo(code);
        ...
        private static string GetCulture_3LetterISOName(string code)
        {
            try
            {
                CultureInfo? cultureinfo = CultureInfo.GetCultureInfo(code);
        ...
        private static string GetCulture_Windows3LetterName(string code)
        {
            try
            {
                CultureInfo? cultureinfo = CultureInfo.GetCultureInfo(code);
